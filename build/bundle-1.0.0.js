(function () {
	...
	/* src/Pythagoras.html generated by Svelte v2.7.0 */
	...
	function create_main_fragment(component, ctx) {
		...
		var if_block = (ctx.check) && create_if_block(component, ctx);
		...
		return {
			c: function c() {
				...
				setAttribute(rect, "width", ctx.w);
				setAttribute(rect, "height", ctx.w);
				setStyle(rect, "fill", ctx.fill);
				setAttribute(g, "transform", g_transform_value = "translate(" + ctx.x + " " + ctx.y + ") " + ctx.rotate);
			},
			...
			p: function p(changed, ctx) {
				...
				if ((changed.x || changed.y || changed.rotate) && g_transform_value !== (g_transform_value = "translate(" + ctx.x + " " + ctx.y + ") " + ctx.rotate)) {
	...
	function create_if_block(component, ctx) {
		...
		return {
			...
			p: function p(changed, ctx) {
				...
				if (changed.maxlvl) { pythagoras_changes.maxlvl = ctx.maxlvl; }
				if (changed.heightFactor) { pythagoras_changes.heightFactor = ctx.heightFactor; }
				if (changed.lean) { pythagoras_changes.lean = ctx.lean; }
				if (changed.nextLeft) { pythagoras_changes.y = -ctx.nextLeft; }
				if (changed.nextLeft) { pythagoras_changes.w = ctx.nextLeft; }
				if (changed.lvl) { pythagoras_changes.lvl = ctx.lvl + 1; }
				pythagoras._set(pythagoras_changes);

				var pythagoras_1_changes = {};
				if (changed.maxlvl) { pythagoras_1_changes.maxlvl = ctx.maxlvl; }
				if (changed.heightFactor) { pythagoras_1_changes.heightFactor = ctx.heightFactor; }
				if (changed.lean) { pythagoras_1_changes.lean = ctx.lean; }
				if (changed.w || changed.nextRight) { pythagoras_1_changes.x = ctx.w - ctx.nextRight; }
				if (changed.nextRight) { pythagoras_1_changes.y = -ctx.nextRight; }
				if (changed.nextRight) { pythagoras_1_changes.w = ctx.nextRight; }
				if (changed.lvl) { pythagoras_1_changes.lvl = ctx.lvl + 1; }
				pythagoras_1._set(pythagoras_1_changes);
			},

			d: function d(detach) {
				pythagoras.destroy(detach);
				pythagoras_1.destroy(detach);
			}
		};
	}
	...
	Pythagoras.prototype._recompute = function _recompute(changed, state) {
		if (changed.heightFactor || changed.w) {
			if (this._differs(state.trigH, (state.trigH = trigH(state)))) { changed.trigH = true; }
		}

		if (changed.w || changed.trigH || changed.lean) {
			if (this._differs(state.A, (state.A = A$1(state)))) { changed.A = true; }
			if (this._differs(state.B, (state.B = B$1(state)))) { changed.B = true; }
		}

		if (changed.left || changed.right || changed.A || changed.B || changed.w) {
			if (this._differs(state.rotate, (state.rotate = rotate(state)))) { changed.rotate = true; }
		}

		if (changed.w || changed.trigH || changed.lean) {
			if (this._differs(state.nextLeft, (state.nextLeft = nextLeft(state)))) { changed.nextLeft = true; }
			if (this._differs(state.nextRight, (state.nextRight = nextRight(state)))) { changed.nextRight = true; }
		}

		if (changed.lvl || changed.maxlvl) {
			if (this._differs(state.fill, (state.fill = fill(state)))) { changed.fill = true; }
		}

		if (changed.w || changed.lvl || changed.maxlvl) {
			if (this._differs(state.check, (state.check = check(state)))) { changed.check = true; }
		}
	};

	/* src/App.html generated by Svelte v2.7.0 */
	...
	var methods = {
	  ...
	  update: throttleWithRAF(function (x, y) {
	    ...
	    var scaleFactor = linear$2()
	      .domain([innerHeight, 0])
	      .range([0, 0.8]);

	    var scaleLean = linear$2()
	...
	function create_main_fragment$1(component, ctx) {
		...
		var pythagoras_initial_data = {
		 	...
		 	heightFactor: ctx.heightFactor,
		 	lean: ctx.lean,
		 	x: (ctx.innerWidth - ctx.baseW) / 2,
		 	y: ctx.innerHeight - ctx.baseW,
		 	lvl: 0,
		 	maxlvl: ctx.currentMax
		 };
		var pythagoras = new Pythagoras({
			root: component.root,
			data: pythagoras_initial_data
		...
		return {
			c: function c() {
				...
				setAttribute(svg, "width", ctx.innerWidth);
				setAttribute(svg, "height", ctx.innerHeight);
			},
			...
			p: function p(changed, ctx) {
				var pythagoras_changes = {};
				if (changed.baseW) { pythagoras_changes.w = ctx.baseW; }
				if (changed.baseW) { pythagoras_changes.h = ctx.baseW; }
				if (changed.heightFactor) { pythagoras_changes.heightFactor = ctx.heightFactor; }
				if (changed.lean) { pythagoras_changes.lean = ctx.lean; }
				if (changed.innerWidth || changed.baseW) { pythagoras_changes.x = (ctx.innerWidth - ctx.baseW) / 2; }
				if (changed.innerHeight || changed.baseW) { pythagoras_changes.y = ctx.innerHeight - ctx.baseW; }
				if (changed.currentMax) { pythagoras_changes.maxlvl = ctx.currentMax; }
				pythagoras._set(pythagoras_changes);
